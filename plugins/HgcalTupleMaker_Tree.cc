...
template <class T>
void HgcalTupleMaker_Tree::TypedBranchConnector<T>::
...
template <class T> 
HgcalTupleMaker_Tree::TypedBranchConnector<T>::
...
beginJob() {
  ...
  edm::ProductSelectorRules productSelectorRules_(pset, "outputCommands", "HgcalTupleMaker_Tree");
    ...
    if(productSelector_.selected(*selection)) {
      ...
      if (branchnames.find( selection->productInstanceName()) != branchnames.end() ) {
        throw edm::Exception(edm::errors::Configuration)
          ...
          << "Exception thrown from HgcalTupleMaker_Tree::beginJob" << std::endl;
      }
      else {
        branchnames.insert( selection->productInstanceName() );
      }

      //Create HgcalTupleMaker_Tree branch
      ...
      case DOUBLE_V_V      :  connectors.push_back( new TypedBranchConnector<vectorVectorDoubles> (selection,   "", tree) ); break;
      ...
      default:
        {
          ...
          throw edm::Exception(edm::errors::Configuration)
            << "class HgcalTupleMaker_Tree does not handle leaves of type " << selection->className() << " like\n"
            ...
            <<   leafstring
            << "Exception thrown from HgcalTupleMaker_Tree::beginJob\n";
        }
      }
    }
  }
